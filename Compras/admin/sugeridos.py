...
class SugeridoLoteAdmin(admin.ModelAdmin):
    ...
    def _es_proveedor(self, request) -> bool:
        return self._get_perfil_proveedor_obj(request) is not None
...
class SugeridoLineaAdmin(admin.ModelAdmin):
    ...
    list_editable = ("sugerido_interno",)  # tu template controla realmente los inputs
    ...
    def _es_proveedor(self, request) -> bool:
        # Tener un perfil_proveedor válido en el user indica que es usuario del proveedor
        return self._get_perfil_proveedor_obj(request) is not None

    def _puede_editar_clasificacion_c(self, request) -> bool:
        """Permite a usuarios internos editar líneas con clasificación C si pertenecen al grupo 'clasificacion_c'."""
        return request.user.groups.filter(name='clasificacion_c').exists()

    # --- NUEVO: detectar si el usuario pertenece al grupo "perfil_interno"
    def _es_interno(self, request) -> bool:
        """Bandera para limitar edición a 'clasificacion' y 'sugerido_interno'."""
    ...
    def confirmar_pedido_icg(self, request, lote_id):
        ...
        # Antes: bloqueaba si era proveedor; ahora: exige ser interno
    ...
    def accion_proveedor_enviar_respuesta(self, request, qs):
        """Solo para proveedores: marcar las líneas como "respondidas" y notificar."""
        perfil_prov = self._get_perfil_proveedor_obj(request)
        if not perfil_prov:
            self.message_user(request, "Acción solo para proveedores.", messages.ERROR)
            return

        # Filtrar solo las líneas del proveedor actual
        ...
        # Marcar estado y enviar notificación
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        puede_editar_clasificacion_c = self._puede_editar_clasificacion_c(request)
        es_interno = self._es_interno(request)
        if es_interno:
            es_proveedor = False

        # Procesamiento de POST (edición masiva)
            ...
            with transaction.atomic():
                for ln in lineas:
                    ...
                    if es_proveedor:
                        ...
                        # Propagar descuentos a todas las líneas del mismo código
                        ...
                        # Usuario interno (no proveedor)
                        # Solo permitimos edición si ES del grupo 'perfil_interno'
                        if not es_interno:
                            continue
                        ...
                        # Edición de sugerido_interno: bloquear si clasificación C y no tiene grupo especial
                        editable_interno = not (cla == 'C' and not puede_editar_clasificacion_c)

                        # --- ÚNICOS CAMPOS PERMITIDOS PARA 'perfil_interno' ---
                        # sugerido_interno
                        m_si = request.POST.get(f'sugerido_interno_{pid}')
                        if m_si is not None and editable_interno:
                            try:
        ...
        try:
            ...
            # Obtener presupuesto_total del proveedor del lote
            ...
            # Cumplimiento presupuesto: suma de todos los sugeridos del proveedor en el mes actual vs presupuesto mensual
            ...
            if lote_id:
                try:
                    ...
                    if lote_obj.proveedor:
                        ...
                        # Sumar todos los costos de líneas del proveedor en el mes actual
                        ...
                        # Calcular porcentaje vs presupuesto mensual
            ...
            # NUEVO: permiso para editar líneas clasificación C (para template)
